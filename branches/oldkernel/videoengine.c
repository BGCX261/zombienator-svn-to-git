...
{
	Fill(0, OVERLAY_ROW, 24, 1, 16); //black

	//score
	Print(11, OVERLAY_ROW, scorestr); 
	PrintInt(21, OVERLAY_ROW, player->killz, true);

	if(player->life <= 0)
    {
		Fill(0, OVERLAY_ROW+1, 24, 2, 16); //black
		Print(0, OVERLAY_ROW, overstr); //game over
		Print(5, OVERLAY_ROW + 1, suboverstr); //retry?
		Print(5, OVERLAY_ROW + 2, suboverstr2); //menu?
		Print(4, OVERLAY_ROW + 1 + game->menupos, choicestr);
	}
	else
	{
		if(game->flags & GAMEPAUSED)
			Print(4, OVERLAY_ROW, pausestr); //paused
		else if(game->flags & (GAMEMULTIHO|GAMEMULTILO) && game->frames&2)
		{
			switch(game->flags&(GAMEMULTIHO|GAMEMULTILO))
			{
				case GAMEMULTILO:
						Print(3, OVERLAY_ROW, doublestr); //DOUBLE
					break;
				case GAMEMULTIHO:
						Print(3, OVERLAY_ROW, triplestr); //TRIPLE
					break;
				case GAMEMULTIHO|GAMEMULTILO:
						Print(3, OVERLAY_ROW, insanestr); //INSANE!!
					break;
			}
		}
		...
		SetTile(i, OVERLAY_ROW, 17);
	}
}

void multidrawoverlay(GameStruct* game, PlayerStruct players[2])
{
	Fill(0, OVERLAY_ROW, 24, 2, 16); //black

	for(int p  = 0; p < 2; p++)	//score
	{
		Print(11 * p, OVERLAY_ROW, scorestr); 
		PrintInt(10 + 11 * p, OVERLAY_ROW, players[p].killz, true);
	}

	if(players[0].life <= 0 && players[1].life <= 0)
	{
		Fill(0, OVERLAY_ROW+1, 24, 2, 16); //black
		Print(8, OVERLAY_ROW + 2, overstr); //game over
		Print(1, OVERLAY_ROW + 1, suboverstr); //retry?
		Print(1, OVERLAY_ROW + 2, suboverstr2); //menu?
		Print(0, OVERLAY_ROW + 1 + game->menupos, choicestr);
	}
	else
	{
		for(int p = 0; p < 2; p++)
		{
			if(game->flags & GAMEPAUSED)
				Print(4, OVERLAY_ROW + 1, pausestr); //paused
			else if(game->flags & (GAMEMULTIHO|GAMEMULTILO) && game->frames&2)
			{
				switch(game->flags&(GAMEMULTIHO|GAMEMULTILO))
				{
					case GAMEMULTILO:
							Print(3, OVERLAY_ROW+1, doublestr); //DOUBLE
						break;
					case GAMEMULTIHO:
							Print(3, OVERLAY_ROW+1, triplestr); //TRIPLE
						break;
					case GAMEMULTIHO|GAMEMULTILO:
							Print(3, OVERLAY_ROW+1, insanestr); //INSANE!!
						break;
				}
			}
			...
			SetTile(i + 12 * p, OVERLAY_ROW + 1, 17);
		}
	}
}
