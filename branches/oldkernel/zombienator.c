...
{
	SetTileTable(tiles);
	SetFontTilesIndex(163);
	SetFontTable(fonts);

	//Screen info horziontal
	mains->x.SCREENTILE = SCREEN_TILE_WIDTH;
	...
	mains->y.SCREENTILE = SCREEN_TILE_HEIGHT;
	...
	mains->x.p = 2;
	mains->y.p = 6;
	SetScrolling(0, 0); //scrolling defaults to 0
...
void init(MainStruct *mains, PlayerStruct players[], char nrplayers, GameStruct *game, ItemStruct zombie[MAX_ZOMBIES], ItemStruct shots[][2])
{
	...
	initgraphics(mains);
	SetOverlay(OVERLAY_PRI_HI+nrplayers); //overlay always on
...
{
	if(player->joypad&BTN_DOWN && !mapcollide(mains, player->x + 5, player->y + 14))
		scrollchar(true, &(mains->y), &(player->y));

	else if(player->joypad&BTN_UP && !mapcollide(mains, player->x + 5, player->y + 11))
		scrollchar(false, &(mains->y), &(player->y));

	if(player->joypad&BTN_LEFT && !mapcollide(mains, player->x + 4, player->y + 13))
		scrollchar(false, &(mains->x), &(player->x));

	else if(player->joypad&BTN_RIGHT && !mapcollide(mains, player->x + 6, player->y + 13))
...
void multimove(PlayerStruct players[2], MainStruct* mains,ItemStruct shots[2][2])
{
	...
	for(int p = 0; p < 2; p++)
	{
		...
		else if(players[p].x >= (mains->x.SCREENTILE - 8) * mains->x.TILE)
			canscroll[1 - p] |= RIGHTBORDER;

		if(players[p].y <= (mains->y.TILE<<3))
			canscroll[1 - p] |= TOPBORDER;
		else if(players[p].y >=(mains->y.SCREENTILE - 8) * mains->y.TILE) 
			canscroll[1 - p] |= BOTTOMBORDER;
	}

	for(int p = 0; p < 2; p++)
	{
		if(players[p].joypad&BTN_DOWN && !mapcollide(mains, players[p].x + 5, players[p].y + 14))
		{
		...
		if(players[p].joypad&BTN_UP && !mapcollide(mains, players[p].x + 5, players[p].y + 11))
		{
		...
		if(players[p].joypad&BTN_LEFT && !mapcollide(mains, players[p].x + 4, players[p].y + 13))
		{
		...
		if(players[p].joypad&BTN_RIGHT && !mapcollide(mains, players[p].x + 6, players[p].y + 13))
		{
...
void rendermap(MainStruct* mains)
{
	for(int i = mains->x.p; i < mains->x.p+4; i++)
	{
		for(int u = mains->y.p; u < mains->y.p+8; u++)
			DrawMap2((i-mains->x.p)<<3,(u-mains->y.p)<<2,graveyard_map[i][u]);
	}
}
...
char singleloop(GameStruct* game)
{
	...
	for(;; game->frames++)
	{
		...
		rendermap(&mains);
		SetScrolling(mains.x.s,mains.y.s);
...
char introscreen(GameStruct* game)
{
	...
	ClearVram();
	memset(sprites, 0, sizeof(struct SpriteStruct)*32);
	SetTileTable(zombienator_title);
	SetFontTilesIndex(145);
	SetFontTable(fonts);
	SetOverlay(OVERLAY_PRI_HI);
	SetScrolling(0, 0);
	game->menupos = counter = 0;
	DrawMap2(0,4, title_map);
	DrawMap2(0,10, intro_map);

	for(;;)
	{
		WaitVsync(1);
		Fill(0,13, SCREEN_TILE_WIDTH, SCREEN_TILE_HEIGHT - 13, 0);
