...
char handlezombie(ItemStruct* zombie, PlayerStruct players[], char nrplayers, MainStruct* main)
{
	...
	else if(abs(deltax[target]) > abs(deltay[target]))
	{
		if(deltax[target] > 0)
		{
			if(!mapcollide(main, zombie->x + 4, zombie->y + 13))
			zombie->x--;
		...
		{
			if(!mapcollide(main, zombie->x + 6, zombie->y + 13))
			zombie->x++;
			else if(deltay[target] >= 0)
			zombie->y--;
			else
	...
	else
	{
		if(deltay[target] > 0)
		{
			if(!mapcollide(main, zombie->x + 5, zombie->y + 11))
			zombie->y--;
			else if(deltax[target] >= 0)
			zombie->x--;
			else
		...
		{
			if(!mapcollide(main, zombie->x + 5, zombie->y + 14))
			zombie->y++;
			else if(deltax[target] >= 0)
			zombie->x--;
			else
